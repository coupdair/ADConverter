...
\begin{itemize}
\item $link$ (shell; \textbf{done})
\textit{or}
\item $\#include$ $"../AD7193/AD7193.h"$ (C/C++; \textbf{todo ?})
\end{itemize}

\paragraph*{gather code:}
\begin{itemize}
\item same functions in all device directories:
\begin{lstlisting}
...
\end{lstlisting}
\textbf{to} local (C) or class member (C++)
\begin{itemize}
\item local (C)
\textit{or}
\item class member (C++)
...
##SOURCE_BROWSER         = YES
##STRIP_FROM_PATH        = ../
##HAVE_DOT               = YES ## sudo apt-get install graphviz
...
add doxygen comment for functions, i.e. $//!$ $short description$.
